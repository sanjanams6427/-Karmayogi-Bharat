...
[1] Audio Extraction — ffmpeg → 16kHz mono WAV
         │
         ▼
[1b] S2ST Fast-Path — SeamlessM4T (Indic→Indic only)
         │  → success: mux + return (skip steps 2–5)
         ▼
[2] ASR — faster-whisper large-v3
         │  Segment merge, VAD, hallucination guard
         │  Nastaliq normalisation (urd/kas/snd)
         │  Per-segment language detection
         ▼
[3] Translation
         │  TM lookup (exact HF → exact Govt → 85% fuzzy)
         │  Token protection (__F__, __NT__, __FMT__)
         │  IndicTrans2 (fine-tuned) → SeamlessM4T → NLLB-200
         │  Glossary injection + quality scoring per segment
         │  Quality gate: score < 0.30 → silence (no wrong-language audio)
         │  Optional: LLM post-edit (Groq/Gemini/OpenRouter)
         ▼
[4] TTS — Parler-TTS Indic Large → MMS-TTS → Coqui XTTS-v2
         │  Per-segment WAV files
         │  Voice cloning (XTTS-v2, optional, consented)
         ▼
[5] Audio Assembly
         │  Segments placed at original timestamps
         │  Fit-to-slot: max 1.35× speed, hard-trim if still over
         │  Duration ratio check (>20% expansion → KB approval flag)
         ▼
[6] Output
         │  ffmpeg mux (dubbed audio → original video)
         │  SRT + VTT subtitle generation
         │  Metadata JSON (quality scores, model versions, git hash)
